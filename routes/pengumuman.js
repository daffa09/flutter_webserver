...
/*panggil Model pengumuman*/
var pengumumanModel = require('../models/pengumuman.js');

/* TAMPIL DATA */
router.get('/', cekToken2an, function(req, res, next) {
    pengumumanModel.findAll().then( data=>{
        res.json({
            status:true,
...
router.post('/', cekToken2an, function(req, res, next) {
    pengumumanModel.create(req.body).then (data=>{
       res.json({
           status:true,
...
router.put('/', cekToken2an, function(req, res, next) {
    pengumumanModel.update(req.body,{
